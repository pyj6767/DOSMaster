...
def get_atom_count(DOS_temp, Labellist):
    # Count how many atoms compose a single DOS line.
    total_atom_number = len(Labellist)
    if isinstance(DOS_temp, list) == True:
        # An already-averaged bundle (_avg) is per-atom DOS, so it counts as 1.
        try:
            ...
            pass
        atom_count = 0
        for element in DOS_temp:
            if element.split('_')[0] == 'Total DOS':
                atom_count += total_atom_number
            else:
                atom_count += 1
        return atom_count
    else:
        if DOS_temp.split('_')[0] == 'Total DOS':
            return total_atom_number
        else:
            return 1

def get_dos_divisor(DOS_temp, Labellist, graph_config):
    # Return the number to divide a DOS line by (1 if division is off for this line).
    if isinstance(DOS_temp, list) == False:
        dos_name = DOS_temp
    else:
        dos_name = ' and '.join(DOS_temp)
    if graph_config.get('divide_by_atom', {}).get(dos_name, False) == True:
        return get_atom_count(DOS_temp, Labellist)
    else:
        return 1
